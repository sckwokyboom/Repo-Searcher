"""Clone repositories and build indexes using the backend pipeline."""
...
def clone_repo_sync(repo_name: str, repos_dir: Path | None = None) -> Path:
    """Clone a repo at HEAD with depth=1."""
...
def index_repo(repo_name: str, repo_path: Path) -> dict:
    """Index a repo using the backend pipeline components."""
    import sys
    ...
    save_indexes(repo_info, chunks, bm25_index, tokenized_corpus, faiss_index, call_graph)
...
def validate_samples(
    ...
    for repo_name, samples in dataset.repos.items():
        ...
        for sample in samples:
            # Check if at least one ground-truth file has chunks in the index
        ...
        if skipped > 0:
            print(f"  {repo_name}: {skipped}/{len(samples)} samples skipped (files not in index)")
...
def clone_and_index_all(dataset: BenchmarkDataset) -> dict[str, dict]:
    """Clone and index all repos in the dataset."""
    results = {}
    for i, repo_name in enumerate(dataset.repos.keys()):
        ...
        try:
            ...
            results[repo_name] = {"repo_id": repo_id_from_name(repo_name), "error": str(e)}
    return results

...
def _print_rich_table(eval_results: EvalResults, k_values: list[int] | None = None):
    ...
    best = {}
    for metric_name in (
        [f"recall_{k}" for k in k_values] + ["mrr"] + [f"hit_{k}" for k in [5, 10]]
    ):
    ...
    if has_method:
        mtable = Table(
            title="Retrieval Benchmark Results (Method-Level)", show_lines=True
        )
...
def plot_results(eval_results: EvalResults, output_dir: Path | None = None):
    ...
    k_values = (
        sorted(eval_results.per_retriever[0].recall_at_k.keys())
        if eval_results.per_retriever
        else []
    )
    ...
    if eval_results.per_repo and len(eval_results.per_repo) > 1:
        ...
        for repo in repos:
            ...
            row = [
                aggs[r].recall_at_k.get(5, 0) if r in aggs else 0 for r in retrievers
            ]
            data.append(row)

        fig, ax = plt.subplots(
            figsize=(max(8, len(retrievers) * 2), max(4, len(repos) * 0.6))
        )
...
def save_results_markdown(
    ...
    retrievers = [agg.retriever for agg in eval_results.per_retriever]
    n_samples = (
        eval_results.per_retriever[0].num_samples if eval_results.per_retriever else 0
    )

...
def _get_indexes(repo_id: str) -> FullIndexes:
    if repo_id not in _index_cache:
        ...
        _index_cache[repo_id] = FullIndexes(
            chunks=chunks,
            bm25=bm25,
            corpus=corpus,
            faiss=faiss_index,
...
class HybridRetriever:
    ...
    def _search_impl(self, query: str, top_k: int = 5) -> dict:
        ...
        candidate_chunks = [
            (chunks[idx], rrf_score) for idx, rrf_score in top_candidates
        ]
        ...
        if seed_ids:
            ...
            for discovery in graph_result["discoveries"]:
                if discovery.chunk.chunk_id not in existing_ids:
                    callers, callees = graph_expander.get_neighbors(
                        discovery.chunk.chunk_id
                    )
